...
namespace test_work
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Подключение c БД установлено");
            }
        }
            ...
            if(sb.Length == 9)
            {
                if (Convert.ToString(sb[1]) != "4" || Convert.ToString(sb[2]) != "3" || Convert.ToString(sb[2]) != "3" || Convert.ToString(sb[7]) != "3" || Convert.ToString(sb[8]) != "2")
                {
                    ...
                    else
                    {
                        ...
                        SqlCommand command = new SqlCommand("INSERT INTO [Table] (Name, Number, Pasport, Login, Password) VALUES (@Name, @Number, @Pasport, @Login, @Password)", sqlConnection);
                        command.Parameters.AddWithValue("Name", textBox1.Text);
                        command.Parameters.AddWithValue("Number", maskedTextBox1.Text);
                        command.Parameters.AddWithValue("Pasport", maskedTextBox2.Text);
                        command.Parameters.AddWithValue("Login", textBox2.Text);
                        command.Parameters.AddWithValue("Password", textBox3.Text);

                        MessageBox.Show("Добвленно записей: " + command.ExecuteNonQuery().ToString());
